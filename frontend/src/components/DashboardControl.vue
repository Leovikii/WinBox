...
<template>
  ...
  <div class="w-full flex-1 flex flex-col justify-center relative">
    <div :class="['w-full bg-[#111] border border-[#222] rounded-xl p-8 flex flex-col gap-6 relative overflow-hidden transition-opacity duration-500', isProcessing ? 'opacity-80 pointer-events-none grayscale' : 'opacity-100']">
      ...
      <div v-if="!coreExists" class="absolute inset-0 bg-[#eab308]/10 blur-2xl pointer-events-none transition-opacity duration-1000"></div>
      <div v-else-if="msg === 'ERROR'" class="absolute inset-0 bg-[#ef4444]/10 blur-2xl pointer-events-none transition-opacity duration-1000"></div>
      <div v-else-if="tunMode && sysProxy" class="absolute inset-0 bg-[#a855f7]/10 blur-2xl pointer-events-none transition-opacity duration-1000"></div>
      <div v-else-if="tunMode" class="absolute inset-0 bg-[#3b82f6]/10 blur-2xl pointer-events-none transition-opacity duration-1000"></div>
      <div v-else-if="sysProxy" class="absolute inset-0 bg-[#10b981]/10 blur-2xl pointer-events-none transition-opacity duration-1000"></div>
      <div class="text-center z-10 cursor-pointer group relative" @click="() => { if (msg === 'ERROR' || errorLog) { emit('open-drawer', 'logs') } else { emit('toggle-service') } }">
        <div :class="[
          ...
          !coreExists ? 'bg-[#eab308]/20' :
          msg === 'ERROR' ? 'bg-[#ef4444]/20' :
          (tunMode && sysProxy) ? 'bg-[#a855f7]/20' :
          tunMode ? 'bg-[#3b82f6]/20' :
          sysProxy ? 'bg-[#10b981]/20' :
          'bg-gray-500/20'
        ]"></div>
        <div class="text-4xl font-black tracking-tighter transition-transform duration-500 whitespace-nowrap relative z-10 group-hover:scale-105" :style="getStatusStyle">{{ getStatusText }}</div>
      ...
      <div class="flex flex-col gap-6 z-10 px-1">
        <div @click="emit('toggle', 'tun')" class="flex items-center justify-between cursor-pointer group select-none py-1">
          <div class="flex items-center gap-4">
            <div :class="['w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-sm transition-colors duration-500', tunMode ? 'bg-[#2563eb] text-white shadow-[0_0_12px_1px_rgba(37,99,235,0.6)]' : 'bg-[#1a1a1a] text-[#444] group-hover:text-[#666] group-hover:bg-[#222]']">
              <i class="fas fa-shield-alt"></i>
            </div>
            <div class="flex flex-col min-w-0">
              <div :class="['text-xs font-bold tracking-wide transition-colors duration-300 whitespace-nowrap', tunMode ? 'text-[#93c5fd]' : 'text-[#555] group-hover:text-gray-400']">TUN MODE</div>
              <div class="text-[9px] text-[#444] whitespace-nowrap group-hover:text-[#555] transition-colors">Virtual Network Interface</div>
            </div>
          </div>
          <div class="rounded-full p-0.5 cursor-pointer transition-colors duration-300 relative w-11 h-6" :class="tunMode ? 'bg-[#2563eb]' : 'bg-[#2a2a2a]'">
            <div class="bg-white rounded-full transition-transform duration-300 shadow-lg absolute top-1 left-1 w-4 h-4" :class="tunMode ? 'translate-x-5' : 'translate-x-0'"></div>
          </div>
        </div>

        <div @click="emit('toggle', 'proxy')" class="flex items-center justify-between cursor-pointer group select-none py-1">
          <div class="flex items-center gap-4">
            <div :class="['w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-sm transition-colors duration-500', sysProxy ? 'bg-[#059669] text-white shadow-[0_0_12px_1px_rgba(5,150,105,0.6)]' : 'bg-[#1a1a1a] text-[#444] group-hover:text-[#666] group-hover:bg-[#222]']">
              <i class="fas fa-globe"></i>
            </div>
            <div class="flex flex-col min-w-0">
              <div :class="['text-xs font-bold tracking-wide transition-colors duration-300 whitespace-nowrap', sysProxy ? 'text-[#6ee7b7]' : 'text-[#555] group-hover:text-gray-400']">SYSTEM PROXY</div>
              <div class="text-[9px] text-[#444] whitespace-nowrap group-hover:text-[#555] transition-colors">Global HTTP Proxy</div>
            </div>
          </div>
          <div class="rounded-full p-0.5 cursor-pointer transition-colors duration-300 relative w-11 h-6" :class="sysProxy ? 'bg-[#059669]' : 'bg-[#2a2a2a]'">
            <div class="bg-white rounded-full transition-transform duration-300 shadow-lg absolute top-1 left-1 w-4 h-4" :class="sysProxy ? 'translate-x-5' : 'translate-x-0'"></div>
          </div>
        </div>
      </div>
    </div>
