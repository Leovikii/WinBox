...
const SystemButton = ({ children, variant = 'normal', loading, icon, className, disabled, ...props }: SystemButtonProps) => {
    ...
    switch (variant) {
        case 'primary':
            variantStyle = "bg-blue-600 text-white hover:bg-blue-500 shadow-[0_0_15px_rgba(37,99,235,0.4)] border border-blue-500";
            break;
        case 'danger':
            variantStyle = "bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20 hover:border-red-500/40 hover:shadow-[0_0_10px_rgba(220,38,38,0.2)]";
            break;
        case 'success':
            variantStyle = "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20";
            break;
        default: // normal
            variantStyle = "bg-[#1a1a1a] border border-[#333] text-gray-300 hover:bg-[#222] hover:border-[#444] hover:text-white hover:shadow-[0_0_10px_rgba(255,255,255,0.05)]";
            break;
    }
...
const SettingsItem = ({ label, children }: { label: string, children: React.ReactNode }) => (
    <div className="flex justify-between items-center py-2 min-h-10 border-b border-[#222]/50 last:border-0">
...
function App() {
    ...
    const handleStartOnBootToggle = async () => {
        ...
        if (res === "Success") {
            ...
            if (newState && !autoConnect) {
                await Backend.SetAutoConnect(true, autoConnectMode);
                setAutoConnect(true);
            }
        }
        else alert(res);
    ...
    const otherProfiles = profiles.filter(p => activeProfile && p.id !== activeProfile.id);
        ...
        <div className="h-screen w-screen relative bg-[#090909] text-white select-none border border-[#222] rounded-xl overflow-hidden font-sans flex flex-col shadow-2xl">
            ...
            <div className={`absolute inset-0 pt-16 px-6 pb-8 flex flex-col justify-between items-center transition-all duration-500 ${isDrawerOpen ? 'scale-95 opacity-50 blur-[2px]' : 'scale-100 opacity-100'}`}>
                ...
                <div className="w-full pt-4">
                    <div className="text-[9px] font-bold text-[#444] mb-2 tracking-widest uppercase ml-1">Active Configuration</div>
                    <div onClick={() => setActiveDrawer('profiles')} className="w-full bg-[#131313] border border-[#222] rounded-2xl p-4 cursor-pointer group relative overflow-hidden h-20 flex items-center transition-all duration-300 hover:border-[#333] hover:shadow-[0_0_20px_rgba(255,255,255,0.03)] active:scale-[0.98]">
                        <div className="flex justify-between items-center w-full z-10 relative">
                            <div className="overflow-hidden mr-4">
                                <div className="text-sm font-bold text-white mb-1 truncate">{activeProfile ? activeProfile.name : "Select Profile"}</div>
                                <div className="text-[10px] text-[#555] font-mono truncate group-hover:text-[#777] transition-colors">{activeProfile && activeProfile.updated ? `Updated: ${activeProfile.updated}` : "Tap to select"}</div>
                            </div>
                            <div className="text-[#333] group-hover:text-blue-500 transition-colors duration-300"><i className="fas fa-chevron-down text-xs"></i></div>
                        </div>
                        {running && <div className="absolute inset-0 bg-blue-500/5 animate-pulse pointer-events-none"></div>}
                    </div>
                </div>
                ...
                <div className="w-full flex-1 flex flex-col justify-center relative">
                    <div className={`w-full bg-[#111] border border-[#222] rounded-4xl p-8 flex flex-col gap-6 relative overflow-hidden transition-all duration-500 ${isProcessing ? 'opacity-80 pointer-events-none grayscale' : 'opacity-100'}`}>
                        ...
                        <div className="h-px bg-[#222]/80 z-10 mx-auto w-[90%]"></div>
                        <div className="flex flex-col gap-6 z-10 px-1">
                            <SwitchRow label="TUN MODE" sub="Virtual Network Interface" active={tunMode} color="blue" icon="fa-shield-alt" onClick={() => handleToggle('tun')} />
                            <SwitchRow label="SYSTEM PROXY" sub="Global HTTP Proxy" active={sysProxy} color="purple" icon="fa-globe" onClick={() => handleToggle('proxy')} />
                        </div>
                    </div>
                </div>

                {/* Bottom Actions */}
                <div className="w-full flex gap-3 z-10 pt-4">
                    <button onClick={Backend.OpenDashboard} disabled={!running} className={`flex-1 h-12 rounded-xl text-xs font-bold tracking-wide border border-transparent transition-all duration-300 active:scale-95 ${running ? "bg-blue-600 text-white hover:bg-blue-500 shadow-[0_0_30px_rgba(37,99,235,0.4)]" : "bg-[#1a1a1a] text-[#444] border-[#222] cursor-not-allowed"}`}>DASHBOARD</button>
                    <button onClick={() => setActiveDrawer('logs')} className={`w-12 h-12 rounded-xl border bg-[#1a1a1a] text-[#666] flex items-center justify-center transition-all duration-300 hover:bg-[#222] hover:text-white active:scale-95 ${msg === "ERROR" ? "border-red-500 text-red-500 bg-red-900/10 shadow-[0_0_15px_rgba(220,38,38,0.3)]" : "border-[#222]"}`}> <i className="fas fa-file-lines"></i></button>
                    <button onClick={() => setActiveDrawer('settings')} className="w-12 h-12 rounded-xl border border-[#222] bg-[#1a1a1a] text-[#666] flex items-center justify-center transition-all duration-300 hover:bg-[#222] hover:text-white active:scale-95"><i className="fas fa-cog"></i></button>
                    <button onClick={quitApp} className="w-12 h-12 rounded-xl border border-[#222] bg-[#1a1a1a] text-[#666] flex items-center justify-center transition-all duration-300 hover:border-red-900/50 hover:text-red-500 hover:bg-red-900/10 active:scale-95"><i className="fas fa-power-off"></i></button>
                </div>
            </div>
            ...
            <div className={`absolute inset-x-0 top-10 bottom-0 z-40 bg-[#090909]/95 backdrop-blur-3xl flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${activeDrawer === 'settings' ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="h-10 shrink-0 flex justify-between items-center px-6 border-b border-[#222]">
                    <h2 className="text-xs font-bold text-[#666] uppercase tracking-widest">System Settings</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar [&::-webkit-scrollbar]:hidden">
                    <div className="bg-[#131313] p-5 rounded-xl border border-[#222] shadow-lg">
                        ...
                        <ExpandableSection isOpen={mirrorEnabled}>
                            <div className="flex justify-between items-center py-2 pl-4 border-l-2 border-[#222]">
                                ...
                                <SystemButton onClick={() => openEditor('mirror')}>EDIT</SystemButton>
                            </div>
                        </ExpandableSection>
            ...
            {/* PROFILES DRAWER */}
            <div className={`absolute inset-x-0 top-10 bottom-0 z-40 bg-[#090909]/95 backdrop-blur-3xl flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${activeDrawer === 'profiles' ? 'translate-y-0' : '-translate-y-full'}`}>
                <div className="h-10 shrink-0 flex justify-between items-center px-6 border-b border-[#222]">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar [&::-webkit-scrollbar]:hidden">
                    ...
                    <div className="bg-[#131313] rounded-2xl border border-[#222] shadow-[0_0_30px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-300">
                        ...
                        >
                            <span className="text-xs font-bold text-gray-400">Current Profile</span>
                            <div className="flex items-center gap-3">
                                <span className="text-xs text-blue-400 font-bold font-mono truncate max-w-37.5">{activeProfile ? activeProfile.name : "None"}</span>
                                {otherProfiles.length > 0 && (
                                    <i className={`fas fa-chevron-down text-xs text-[#444] transition-transform duration-300 ${isProfileListExpanded ? 'rotate-180' : ''}`}></i>
                                )}
                            </div>
                        </div>

                        {/* Dropdown: Other Profiles */}
                        <ExpandableSection isOpen={isProfileListExpanded && otherProfiles.length > 0}>
                            <div className="border-t border-[#222] bg-[#0f0f0f]">
                                {otherProfiles.map(p => (
                                    <div key={p.id} className="p-3 border-b border-[#222]/50 last:border-0 flex justify-between items-center hover:bg-[#161616] transition-colors pl-6">
                                        <div className="overflow-hidden pr-3">
                                            <div className="text-xs font-bold text-gray-300 truncate mb-0.5">{p.name}</div>
                                            <div className="text-[9px] text-[#555] truncate font-mono">{p.url}</div>
                                        </div>
                                        <div className="flex gap-2">
                                            <SystemButton onClick={(e) => switchProfile(p.id, e)}>SWITCH</SystemButton>
                                            <button onClick={(e) => deleteProfile(p.id, e)} className="w-7 h-7 rounded-lg text-[#444] hover:text-red-500 hover:bg-red-500/10 transition-colors flex items-center justify-center">
                        ...
                        {/* Footer: Update Row (Merged) */}
                        <div className="px-4 pb-4 pt-0">
                            <div className="flex justify-between items-center h-10 border-t border-[#222] pt-3 mt-1">
                    ...
                    {/* Add Profile Row */}
                    <div className="bg-[#131313] p-4 rounded-2xl border border-[#222] shadow-lg flex justify-between items-center">
                        <span className="text-xs font-bold text-gray-400">Add New Profile</span>
                        <SystemButton variant='primary' icon="fa-plus" onClick={() => setShowAddProfileModal(true)}>ADD</SystemButton>
                    </div>
                </div>
            </div>

            {/* ADD PROFILE MODAL */}
            <div className={`absolute inset-0 z-80 bg-black/60 backdrop-blur-sm flex items-center justify-center transition-opacity duration-300 ${showAddProfileModal ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                <div className={`w-[85%] bg-[#111] border border-[#333] rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 ${showAddProfileModal ? 'scale-100' : 'scale-95'}`}>
                    <div className="h-10 flex justify-between items-center px-4 border-b border-[#222] bg-[#090909]">
                        <h2 className="text-xs font-bold text-[#666] uppercase tracking-widest">NEW CONFIG</h2>
            ...
            <div className={`absolute inset-0 z-80 bg-black/60 backdrop-blur-sm flex items-center justify-center transition-opacity duration-300 ${showEditor ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                <div className={`w-[90%] h-[70%] bg-[#111] border border-[#333] rounded-2xl shadow-2xl flex flex-col overflow-hidden transform transition-all duration-300 ${showEditor ? 'scale-100' : 'scale-95'}`}>
                    <div className="h-10 shrink-0 flex justify-between items-center px-4 border-b border-[#222] bg-[#090909]">
                        ...
                        <div className="flex gap-2">
                            <SystemButton onClick={resetEditor} className="text-yellow-500 border-yellow-900/30 hover:bg-yellow-900/10">RESET</SystemButton>
                            <SystemButton onClick={() => setShowEditor(false)}>CANCEL</SystemButton>
                            <SystemButton variant={saveBtnText === "SAVED" ? 'success' : 'primary'} onClick={saveEditor}>{saveBtnText}</SystemButton>
                        </div>
                    </div>
            ...
            {/* LOGS DRAWER */}
            <div className={`absolute inset-x-0 bottom-0 top-10 z-60 bg-[#090909]/95 backdrop-blur-3xl flex flex-col transition-transform duration-500 ${activeDrawer === 'logs' ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="h-12 border-b border-[#222] flex items-center justify-between px-6">
                    <h2 className="text-xs font-bold text-[#666] uppercase tracking-widest">LOGS</h2>
                    <SystemButton onClick={() => setActiveDrawer('none')}>CLOSE</SystemButton>
                </div>
                <div className="flex-1 overflow-y-auto p-6 bg-[#050505] custom-scrollbar [&::-webkit-scrollbar]:hidden">
                    <pre className="text-[10px] text-gray-400 font-mono whitespace-pre-wrap break-all">{errorLog || "No logs."}</pre>
                </div>
                <div className="p-4 border-t border-[#222] flex justify-end">
                    <SystemButton onClick={copyLog} variant={copyState === "COPIED!" ? 'success' : 'normal'}>{copyState}</SystemButton>
                </div>
            </div>
        </div>
    );
