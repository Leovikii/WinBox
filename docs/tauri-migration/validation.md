...
- 限制/下一步：本地不能验证 GitHub cache 命中或 secret 注入。维护者配置与 `src-tauri/tauri.conf.json` updater 公钥匹配的 `TAURI_SIGNING_PRIVATE_KEY`；加密密钥另配 password。推送后的 PR Action 验证 unsigned NSIS；只有 secret 可用且发布 tag/version 已确认后，合并 `main` 才会触发 signed NSIS 与 Release。MIG-014/MIG-042 继续保持 `review`。
